...
export default function AdminSettings() {
    ...
    const Section = ({ icon: Icon, title, children, color = '#D946EF' }) => (
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="glass-card mb-6">
            <div className="flex items-center gap-3 mb-5">
                ...
                <h3 className="text-base font-semibold" style={{ color: '#F8FAFC' }}>{title}</h3>
            </div>
            {children}
        </motion.div>
    );
    ...
    const Field = ({ label, name, type = 'text', placeholder, note }) => (
        <div>
            ...
            />
            {note && <p className="text-xs mt-1" style={{ color: '#64748B' }}>{note}</p>}
        </div>
    );
    ...
    return (
        <div className="max-w-3xl">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                <div>
                    <h1 className="text-2xl font-bold" style={{ color: '#F8FAFC' }}>System Settings</h1>
                    <p className="text-sm mt-1" style={{ color: '#94A3B8' }}>Configure platform settings</p>
                </div>
            ...
            {/* Info Banner */}
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-card mb-6 flex items-start gap-3" style={{ borderLeft: '3px solid #06B6D4' }}>
                <Info size={18} color="#06B6D4" className="mt-0.5 shrink-0" />
            ...
            {/* Stripe Integration */}
            <Section icon={CreditCard} title="Payment Integration (Stripe)" color="#8B5CF6">
            ...
            {/* Membership Tiers */}
            <Section icon={Crown} title="Membership Tier Thresholds" color="#F59E0B">
            ...
            {/* Loyalty Settings */}
            <Section icon={Shield} title="Loyalty & Points" color="#22C55E">
                <div className="grid sm:grid-cols-2 gap-4">
            ...
            {/* Event Defaults */}
            <Section icon={Settings} title="Event Defaults" color="#06B6D4">
                <div className="grid sm:grid-cols-2 gap-4">
