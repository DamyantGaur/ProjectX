...
export default function UserEvents() {
    ...
    return (
        <div>
            <div className="mb-8">
                <h1 className="text-2xl font-bold" style={{ color: '#F8FAFC' }}>Events</h1>
                <p className="text-sm mt-1" style={{ color: '#94A3B8' }}>Browse and join upcoming events</p>
            </div>
                    ...
                    return (
                        <motion.div key={e.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="glass-card flex flex-col">
                            <div className="flex justify-between items-start mb-2">
                                <h3 className="font-semibold flex-1" style={{ color: '#F8FAFC' }}>{e.title}</h3>
                                <span className={`badge text-xs ${e.price > 0 ? 'badge-vip' : 'badge-success'}`}>{e.price > 0 ? `$${e.price}` : 'Free'}</span>
                            </div>
                            <p className="text-xs mb-4 flex-1" style={{ color: '#94A3B8' }}>{e.description}</p>
                            ...
                            <div className="flex gap-2">
                                {e.tags?.map((t, j) => (
                                    <span key={j} className="text-xs px-2 py-0.5 rounded-full" style={{ background: 'rgba(139,92,246,0.1)', color: '#8B5CF6', border: '1px solid rgba(139,92,246,0.2)' }}>{t}</span>
                                ))}
                            </div>
            ...
            {events.length === 0 && <p className="text-center py-16 text-sm glass-card" style={{ color: '#64748B' }}>No active events right now. Check back later!</p>}
        </div>
    );
}
