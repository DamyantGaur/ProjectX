...
export default function UserRewards() {
 ...
 if (loading) return <div className="flex items-center justify-center h-64 w-8 h-8 rounded-full border-2"><motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity }} style={{ borderColor: 'rgba(201, 169, 110, 0.2)', borderTopColor: 'var(--color-accent-gold)' }} /></div>;
 ...
 <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-card text-center p-7">
 ...
 <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-card text-center p-7">
 ...
 <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-card text-center p-7">
 ...
 return (
 <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 * i }} className="glass-card p-7">
 ...
 <button onClick={() => handleRedeem(r.type, r.points)} disabled={!canAfford || isThisRedeeming || redeemingType !== null} className="text-xs px-4 py-2 rounded-lg font-medium transition-all duration-200"
 style={{
 ...
 {history.map((txn, i) => (
 <div key={i} className={`flex items-center gap-3 py-3 ${i < history.length - 1 ? "border-b border-white/5" : ""}`}>
 <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${txn.points > 0 ? "bg-[var(--color-accent-emerald)]/10" : "bg-[var(--color-accent-rose)]/10"}`}>
