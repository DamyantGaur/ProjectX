...
export default function UserPasses() {
    ...
    const statusConfig = {
        active: { color: '#22C55E', icon: CheckCircle, badge: 'badge-success' },
        used: { color: '#F59E0B', icon: Clock, badge: 'badge-warning' },
        expired: { color: '#EF4444', icon: XCircle, badge: 'badge-danger' },
        revoked: { color: '#EF4444', icon: XCircle, badge: 'badge-danger' },
    };
    ...
    return (
        <div>
            <div className="mb-8">
                <h1 className="text-2xl font-bold" style={{ color: '#F8FAFC' }}>My QR Passes</h1>
                <p className="text-sm mt-1" style={{ color: '#94A3B8' }}>{passes.length} passes total</p>
            </div>
            ...
            {selectedPass && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.7)' }} onClick={() => setSelectedPass(null)}>
                    <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="glass-card max-w-sm w-full text-center neon-glow" onClick={e => e.stopPropagation()}>
                        <h3 className="font-semibold mb-2" style={{ color: '#F8FAFC' }}>{selectedPass.event_title}</h3>
                        ...
                        <div className="my-6 flex justify-center">
                            {selectedPass.qr_image_base64 ? (
                                <div className="p-4 rounded-2xl qr-pulse" style={{ background: '#020005' }}>
                                ...
                                <div className="w-48 h-48 rounded-2xl flex items-center justify-center qr-pulse" style={{ background: '#020005', border: '2px solid rgba(217,70,239,0.3)' }}>
                                    <QrCode size={80} color="#D946EF" />
                                </div>
                            )}
                        </div>

                        <div className="space-y-2 text-xs mb-4" style={{ color: '#94A3B8' }}>
                            <p>Type: <span className="font-medium" style={{ color: '#F8FAFC' }}>{selectedPass.qr_type === 'one_time' ? 'Single Use' : 'Multi-Use'}</span></p>
                            <p>Scans: <span className="font-medium" style={{ color: '#F8FAFC' }}>{selectedPass.scan_count} / {selectedPass.max_scans}</span></p>
                            <p>Tier: <span className="font-medium capitalize" style={{ color: '#D946EF' }}>{selectedPass.membership_tier}</span></p>
                        </div>

                        <p className="text-xs font-mono break-all px-2 py-2 rounded-lg mb-4" style={{ background: 'rgba(139,92,246,0.1)', color: '#64748B', border: '1px solid rgba(139,92,246,0.15)' }}>{selectedPass.token}</p>
                    ...
                    return (
                        <motion.div key={pass.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="glass-card cursor-pointer hover:scale-[1.02] transition-transform" onClick={() => setSelectedPass(pass)}>
                            ...
                            <div className="flex items-center justify-center py-4">
                                <div className={`w-24 h-24 rounded-xl flex items-center justify-center ${pass.status === 'active' ? 'qr-pulse' : ''}`} style={{ background: 'rgba(217,70,239,0.05)', border: `1px solid ${cfg.color}30` }}>
            ...
            {passes.length === 0 && <p className="text-center py-16 text-sm glass-card" style={{ color: '#64748B' }}>No passes yet. Browse events to get your first pass!</p>}
        </div>
    );
}
