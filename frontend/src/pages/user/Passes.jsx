...
export default function UserPasses() {
    ...
    return (
        <div>
            ...
            {/* QR Detail Modal */}
            {selectedPass && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-50 flex items-center justify-center p-4" className="bg-black/80 backdrop-blur-sm" onClick={() => setSelectedPass(null)}>
                    <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="glass-card max-w-sm w-full text-center neon-glow" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="my-6 flex justify-center">
                            {selectedPass.qr_image_base64 ? (
                                <div className="p-4 rounded-2xl qr-pulse" className="bg-black/40 border border-white/5">
                                ...
                                <div className="w-48 h-48 rounded-2xl flex items-center justify-center qr-pulse" className="bg-black/40 border-2 border-[var(--color-accent-gold)]/30">
                        ...
                        <p className="text-xs font-mono break-all px-2 py-2 rounded-lg mb-4" className="bg-white/5 text-[var(--color-text-muted)] border border-white/10">{selectedPass.token}</p>
                    ...
                    return (
                        <motion.div key={pass.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="glass-card cursor-pointer hover:scale-[1.02] transition-transform" onClick={() => setSelectedPass(pass)}>
                            ...
                            <div className="flex items-center justify-center py-4">
                                <div className={`w-24 h-24 rounded-xl flex items-center justify-center ${pass.status === 'active' ? 'qr-pulse' : ''}`} className="bg-white/5 border border-white/10">
